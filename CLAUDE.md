...
- `src/render/` — `turns.ts` converts records → `Turn[]` for both dialects; `markdown.ts` / `html.ts` (hand-rolled md→html, no third-party md lib) / `screenshot.ts` (PNG via system Chrome, zero puppeteer dep); `theme.ts` is the single source of truth for Dossier visual tokens (warm bone canvas, paper cards, off-black text/actions, muted pastel semantic accents; dark mode via `prefers-color-scheme`).
