...
保留 `src/server/page.ts` 里的 JS/DOM 锚点：IDs `sess`、`sbanner`、`list`、`preview`、`status`、`count`、`all`、`none`、`done`、`tools`、`loading`、`limit`、`sid`（会话 id 胶囊，点击复制 resume 命令）；属性 `data-a`、`data-f`；iframe internals `pv-turn-*`、`pv-sub`；row/list anchors `.row .prev`、`.tag`、`.warn`（含 `.wicon` SVG 标记）。loading overlay 是实色 `--bg-subtle` 覆盖 main（非半透明材质），mark 脉动动画尊重 `prefers-reduced-motion`。
