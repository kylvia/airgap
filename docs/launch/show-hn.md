...
**Why local session processing.** The official cross-machine story (teleport / cloud / remote control) explicitly excludes people who use a raw API key, Bedrock/Vertex, a self-hosted LLM gateway, or are ZDR (zero-data-retention) users. That's exactly the population that has *no* first-party way to move a session between machines. I'm one of them. airgap does not upload session content; you move the `.ccpack` over whatever channel you already trust (USB, internal transfer, encrypted mail). Interactive runs may check `registry.npmjs.org` for update metadata no more than once every 24 hours unless you disable that check with `AIRGAP_NO_UPDATE_CHECK=1` or `~/.airgap/config.json`.
