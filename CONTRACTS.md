...
- `src/render/image-data.ts` — 统一识别 `Turn.userImages` 以及标题、用户、助手和工具文本中的合法 PNG/JPEG/WebP/GIF base64 data URI；供 CLI `show` 与 Share 共用风险门禁
- `src/render/markdown.ts` — `export function renderMarkdown(turns: Turn[], meta: { title: string; date: string }): string`；保留文字占位符，并从最终 Markdown 中移除支持格式的内联图片字节
...
- `src/commands/show.ts`
  ...
  - HTML/PNG 含 `Turn.userImages` 或渲染文本中的合法 `data:image` 字节时，先提示图片像素无法扫描/脱敏：交互环境人工确认，非交互环境拒绝；`--yes` 表示明确接受图片风险。Markdown 会移除支持格式的图片字节，因此不触发图片确认
...
- 文字检测器和 redactor 不读取图片像素；任何新增的图片导出路径必须复用 `src/render/image-data.ts` 的显式人工风险确认，不能只检查 `Turn.userImages`，也不能把 `--redact` 解释成图片已脱敏。
