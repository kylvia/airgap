...
This file is the shared source of truth for coding agents working in this repository. `CLAUDE.md` is a symlink to this file so Codex and Claude Code load the same project guidance.
...
- `src/render/` — `turns.ts` converts records → `Turn[]` for both dialects and retains validated inline user images as `data:` URLs; `image-data.ts` detects image bytes across structured images and rendered text and strips supported data images from Markdown; `html.ts` embeds only validated PNG/JPEG/WebP/GIF data images (md→html via markdown-it: `html:false`, XSS-safe and zero-external); `screenshot.ts` renders PNG via system Chrome with zero puppeteer dependency. `theme.ts` is the single source of truth for Dossier visual tokens (warm bone canvas, paper cards, off-black text/actions, muted pastel semantic accents; dark mode via `prefers-color-scheme`).
...
- **Image pixels are outside the text detector/redactor boundary.** HTML/PNG export containing `Turn.userImages` or supported inline `data:image` bytes in rendered text must require explicit manual-risk acceptance; `--redact` never implies that image contents are safe. Markdown must strip those bytes. Keep CLI `show` and Share export behavior on the shared `image-data.ts` policy.
- **Keep the shared assistant package collision-free** — Claude and Codex both consume `plugins/airgap/`, but Codex auto-discovers a root `hooks/hooks.json`; Claude-only hooks therefore stay under `claude-hooks/` and are referenced explicitly by the Claude manifest. Never give a `commands/*.md` file the same stem as a `skills/*` directory, because Claude gives the skill precedence and silently shadows the command.
