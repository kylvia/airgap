...
- `src/render/` — `turns.ts` converts records → `Turn[]` for both dialects and retains validated inline user images as `data:` URLs; `markdown.ts` omits image bytes while `html.ts` embeds only validated PNG/JPEG/WebP/GIF data images (md→html via markdown-it: `html:false`, XSS-safe and zero-external); `screenshot.ts` renders PNG via system Chrome with zero puppeteer dependency. `theme.ts` is the single source of truth for Dossier visual tokens (warm bone canvas, paper cards, off-black text/actions, muted pastel semantic accents; dark mode via `prefers-color-scheme`).
...
- **Image pixels are outside the text detector/redactor boundary.** HTML/PNG export that embeds `Turn.userImages` must require explicit manual-risk acceptance; `--redact` never implies that image contents are safe. Markdown must omit image bytes. Keep CLI `show` and Share export behavior aligned.
