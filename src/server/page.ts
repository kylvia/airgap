...
function renderList() {
  ...
  for (const t of detail.turns) {
    ...
    row.onclick = (e) => { if (e.target !== cb) syncPreview(t.index); };
...
// 一次性把所有轮渲染进 iframe（各自带 id），之后靠 syncPreview 切「正常/降淡」+ 滚动，
// 不再每次勾选都重载 iframe——更顺滑，且能滚动到任意轮（未勾选的降淡显示，不会进导出）。
function buildPreviewShell() {
  const blocks = detail.turns
    .map((t) => '<div id="pv-turn-' + t.index + '">' + t.html + '</div>')
    .join("\\n");
  // <base target="_blank">：会话内容里的链接在新标签打开，绝不让预览 iframe 本身被导航走
  // （否则点一下相对链接，iframe 就跳到 share server 的 404，预览直接报废）。
  // .pv-dim 是预览 shell 专属样式（未勾选轮降淡），不进 CHAT_CSS，导出物里不存在。
  const doc = '<!DOCTYPE html><html><head><meta charset="UTF-8"><base target="_blank"><style>' + CHAT_CSS + '</style>'
    + '<style>.pv-dim { opacity: 0.35; transition: opacity 120ms ease; }</style></head><body><div class="wrap">'
...
// 按当前 selected 切换各轮「正常/降淡」、刷新预览头部计数；scrollTo 非空时平滑滚到该轮。
// 未勾选的轮降淡而非隐藏：点任意行都能滚过去看内容，导出仍只含勾选轮。
