...
Share 链路支持英文和简体中文。语言优先级依次是 `--lang`、`AIRGAP_LANG`、`~/.airgap/config.json`、系统首选界面语言；不支持的语言回落英文。自动探测在 macOS 读取 `AppleLanguages`，Windows 读取 Node.js `Intl` 语言，Linux 依次读取 `LC_ALL` / `LC_MESSAGES` / `LANG`。终端提示、选择器、API 提示及 HTML/Markdown/PNG 导出物始终使用同一次解析出的语言：
