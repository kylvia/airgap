...
HTML/PNG 可能包含用户消息里的内嵌原图或文本中的 `data:image` Markdown，但 airgap 无法检查或脱敏图片像素里的密钥，因此含这些图片字节的导出还需要单独人工确认。非交互环境中，请先检查图片，再用 `--yes` 明确接受风险。Markdown 会移除支持格式的内联图片字节，只保留文字占位符。
...
- **图片内容不会被扫描或脱敏。** HTML/PNG 只在单独人工确认风险后保留支持的用户内嵌图片和 `data:image` Markdown；`--redact` 只处理文字，不处理像素。Markdown 会移除支持格式的内联图片字节。
